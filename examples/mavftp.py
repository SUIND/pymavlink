...
class MAVFTP:  # pylint: disable=missing-class-docstring, too-many-instance-attributes
    def __init__(self, master, target_system, target_component):
        ...
        self.master.mav.set_callback(self.master_callback, self.master)
    ...
    def master_callback(self, m, _master):
        '''process mavlink message m on master, sending any messages to recipients'''
        try:
            self.mavlink_packet(m)
        except Exception as msg:
            exc_type, exc_value, exc_traceback = sys.exc_info()
            if self.ftp_settings_debug > 3:
            ...
            elif self.ftp_settings_debug > 1:
